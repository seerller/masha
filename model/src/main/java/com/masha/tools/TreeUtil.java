...
public class TreeUtil {
	...
	/**
	 * 根据权限菜单集合转换成前端可视view对象集合
	 * @param menus
	 * @param root
	 * @return
	 * @throws Exception
	 */
	public List<AuthView> buildTree(List<Auth> menus, int root){
		try{
			List<AuthView> trees = new ArrayList<>();
			AuthView node;
			for (Auth menu : menus) {
				node = new AuthView();
				BeanUtils.copyProperties(node,menu);
				node.setId(menu.getAuthId());
				node.setParentId(menu.getAuthPid());
				trees.add(node);
			}
			return TreeUtil.buildByLoop(trees, root);
		}catch (Exception e){
			log.info("类型转换异常");
			throw new LogicException("系统未知错误,请反馈给管理员");
		}
