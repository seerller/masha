package com.masha.controller.commen;
...
//    }
    ...
    public Object resultSuccess(Object object, String fontType){
        if(StringUtils.isNotEmpty(fontType)){
            if (fontType.equals("F")) {
                return JSON.parseObject(ZhConverterUtil.convertToTraditional( JSON.toJSONString(new MessageBean(SUC,"操作成功",object))));
            }else{
                return new MessageBean(SUC,"操作成功",object);
            }
        }
    ...
    /*public User resultUser(){
        return (User) resultSessionValue( "user");
    }*/
}
